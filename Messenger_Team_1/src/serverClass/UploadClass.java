...
public class UploadClass {
	...
	public HashMap<String,Object> uploadProc(HashMap<String,Object> request) {
		...
		try {
			FileOutputStream fout = new FileOutputStream("src/uploadFiles/"+fileName);
			int num =0;
			System.out.println("fileInfo : "+fileInfo.size());
			for(int i = 0;i<fileInfo.size();i++) {
				System.out.println("fileInfo.get: "+fileInfo.get(i));
				fout.write(fileInfo.get(i));
				if(i%(int)(fileInfo.size())==0) {
					if(num==100) {	
